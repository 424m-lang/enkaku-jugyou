...
- 音声ありの3000台で配信が止まりました。ただし**測定側のPCが先に苦しくなっていた可能性があり**、
  サーバの限界と断定はできません。2000台までは余裕がありました

### 一斉入室で、人数の二乗にしないこと

授業のはじめに全員が一度に入ってきます。ここは**1人あたりの処理を一定に保つ**必要があり、
うっかり「全員まとめて」を触ると、たちまち人数の二乗になります。
...
> 上の表は前後を続けて測った比較なので**差は意味がありますが**、
> 秒数そのものは環境で1.5〜2倍ぶれます。
> 見るべきは「1件あたりが台数につれて伸びないこと」のほうです。

> **ここを触るときの目安**: 接続1本の処理の中で「全参加者」「全ソケット」を
> なめる処理を足さないこと。`fetchSockets()` と、lessonId で引く全件SELECTが特に危ないです。
