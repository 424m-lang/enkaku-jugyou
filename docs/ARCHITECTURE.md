...
コードを変えるときに事故になりやすい箇所を [触るときに気をつけること](#触るときに気をつけること) にまとめてあるので、
そこだけは先に目を通してください。
...
クライアントとサーバのどちらを変えるときも、まずここを見てください。
...
4. `client/dist` があれば静的配信を登録し、**SPAフォールバック**を設定
   - `/api` で始まるものだけ 404 を返し、それ以外は `index.html` を返します
...
授業開始時の一斉入室で普通に起きる条件なので、意図的な処理として残してください。
...
記録用の形式は、復活しても**戻しません**。戻しても録音がもう1つのパートに分かれるだけで、得るものがないためです。
...
`client/src/lib/pdf.ts` の `MAX_CACHED_PAGES = 6` を大きくしたり、全ページ事前描画に戻したりしないでください。
...
「volatile を付けたから通信量が抑えられている」という前提でコードを書かないでください。
...
### SPAフォールバックは 404 を隠します

存在しないアセットを要求すると、`index.html` が **200** で返ります。
コード分割のチャンクが1つ欠けると、エラーではなく**白い画面**になります。
ビルド構成を変えたら、チャンクが実際にJSとして配信されているかを確認してください。
...
検証では「機械学習」に引かれて、実際の発話「今回の授業」が**「機械の授業」と誤認識**されました。
