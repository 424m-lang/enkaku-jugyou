...
**スライドへの書き込みは、増えた点だけを送ります**（`SlideCanvas.tsx` / `applyStrokeProgress`）。
以前は66msごとに「その時点までの点列を丸ごと」送り直しており、座標も丸めていなかったため、
線が長くなるほど1回が重くなっていました。

| 3秒の線・端末1台あたり | 直す前 | 直したあと |
|---|---|---|
| 合計 | 163KB | 11KB |
| ピーク | 751kbps | 32kbps（**線の長さによらず一定**） |

座標は `posFromClient` で小数4桁へ丸めます。幅1600pxのスライドで0.16pxにあたるので、
描画にも当たり判定にも影響しません。DBに残るストロークも小さくなります。

**この経路は volatile なので断片が落ちます。** 落ちた場合は繋がらない断片を捨てて、
いまの形をそのまま残します（描き終わりに届く確定ストロークで正しい形に戻る）。
**無理に繋ぐと、実際には引いていない線が生徒の画面に出ます。**

**教室のモニターで受ける構成**

生徒の端末を「教室から参加」（音声OFF）にすると、音声の送り先はモニターの台数だけになります。
**ただしスライドへの書き込みは、音声を切っている生徒の端末にも届きます**（同じ room へ配るため）。
音声だけを数えて「教室数ぶん」と見積もると、実際より1桁小さくなります。
...
書き込みの分は**線を引いているあいだだけ**なので、ふだんは音声の列だけになります。

**この構成で効いてくるのは音声ではありません。** 端末の総数に比例する3つ、
すなわち参加時のPDF、書き込みとポインタ、一斉入室の待ち時間です。

**校内のPCでサーバを動かす場合、この通信は校内で完結します。**
インターネット回線へ出るのはAIの呼び出しだけです。
...
ダミーの文字起こしと要約を返します。**外部へは何も送りません。**

| 動くもの | 動かないもの |
|---|---|
| 配信・書き込み・反応・コメント・タスク・アンケート・録音・振り返りの再生・字幕 | コメントの「関連する説明」・AI要約・復習動画の章分け |

用途は2つです。**キーを持たない状態で全画面の動作を確認すること**と、
**外部へ送れない環境で配信だけを使うこと**です。
前者では、話速グラフのUI確認用に10秒ごとの擬似セグメントも作ります（`ai/transcribe.ts`）。

字幕はこの設定と無関係に動きます。ブラウザの音声認識を使っており、サーバを通らないためです。
