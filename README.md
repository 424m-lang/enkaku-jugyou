...
| 画面 | 説明 |
|---|---|
| `/check` | 授業前に、端末1台の接続、回線速度、音声形式、映像形式を確認します。使用できる機能の目安を表示し、音声出力はテスト音で確認します。ログインは不要です。先生の端末では、送信速度と実際に生成する音声の通信量も測定できます |
...
4. **授業の開始** — 「授業を開始」を押すと、配信と録音が始まります。スライド移動・ポインターなどは、この時刻以降の操作を記録します。開始前の書き込みは、開始0分時点の状態として記録します。
...
現在の構成では、サーバ本体と3GBの永続ディスクが**月1,200円ほど**です。
このほかに、AI利用料と、Renderの無料枠を超えた外向き通信量が加わります。
内訳と試算条件は [導入を検討する方へ「月額の目安」](docs/導入を検討する方へ.md#月額の目安クラウド運用) にあります。

AI利用料の多くは文字起こしです。**Whisperへ送信した音声の分数で課金される**ため、
費用は授業の長さ、処理回数、繋ぎ目の重なりによって変わります。
5分ごとの定期処理だけなら重なりは約5%ですが、コメント受信時の追加処理によって増える場合があります。
処理する音声範囲がほぼ無音と判定された場合は、その範囲をWhisperへ送信しません。
短い発話を含む範囲は、無音部分を含む長さに応じて課金されます。
要約とコメント分析の費用は、コメントの件数と文字数によって変わります。
