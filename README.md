...
| **教室モニター** | 教室の大画面・電子黒板に投影する表示専用ページ。参加用QRコードは授業前に自動で出るほか、授業中も操作バーから出せる<!-- 2026-08-28: 授業中のQRを追記 --> |
...
| **反応タブ** | ボタン反応とコメントを1本の時系列に並べ、それぞれの前後だけを切り出して聞ける。アンケートとタスクの要旨もここに出る |
<!-- ▼ 2026-08-28 に追記（アンケート・タスクのタブ）／編集どうぞ -->
| **アンケートタブ** | 設問ごとの集計。選択式は円グラフ（未回答も区画として入る）、記述式は名前つきの一覧。同じ質問を繰り返した場合は回ごとに並ぶ |
| **タスクタブ** | タスクごとの完了人数と、はじめ・半数・最後の完了時刻。押してすぐ取り消した完了は数えない |
<!-- ▲ここまで -->
