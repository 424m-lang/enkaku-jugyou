...
高知工科大学 データ＆イノベーション学群のPBL（課題解決型学習：Project Based Learning）の一環として開発したWebアプリです。
遠方の学校への配信・自宅受講・複数拠点への同時配信といった遠隔授業で、
生徒の理解や進行度合いを、先生が授業中に把握しやすいよう支援します。

生徒がボタン・コメント・タスク・アンケートで意思表示できます。
...
![先生の授業画面。スライドへの書き込みと、生徒から届いたコメントや反応が同じ画面に並ぶ](docs/images/teach.png)
...
音声・スライド・書き込みを配信し、音声のみの配信には32kbpsを使用します。
カメラ映像を受信するかどうかは、端末ごとに選択できます。
**速度制限中（128kbps程度）でも接続を維持すること**を設計目標にしています。
`/check`では、端末1台の回線速度と対応形式を測定し、利用できる機能の目安を表示します。
短時間の測定であるため、速度制限中の回線では授業中の音声も確認します。
...
使用するAI機能は、授業を開始する前に選択できます。
コメント整理を使用する場合は、文字起こしと分析がバックグラウンドで実行されます。
...
| 機能 | 説明 |
|---|---|
| **スライド配信** | PDFをアップロードすると、ページ送り・書き込み・ポインターがリアルタイムで同期されます。画面のボタン・矢印キー・スライド一覧からページを移動できます |
...
| **タスク進捗** | 生徒がチェックした進捗を棒グラフで表示します。未完了の生徒は、進捗を変更していない時間が長い順に8人まで表示します |
| **アンケート** | 選択式・段階評価・記述式の設問を表示します。選択式と段階評価の集計結果は、生徒へ公開するかどうかを選択できます |
| **コメント整理**（任意） | コメントのカードに「質問」「困りごと」「授業外」「意見・感想」のチップを1つ付けます。質問には「説明した内容」「説明前の内容」を補足します。時刻が近く、同じ話題のコメントは1枚のカードにまとめます。カードは対応済みにするまで表示されます（[操作方法](docs/先生向けガイド.md#生徒の反応を見る)） |
| **AI機能設定** | コメント整理、字幕履歴の補正、授業全体の要約、復習動画の自動章分けを授業ごとに選択できます。設定は授業開始前に行います |
| **教室モニター** | 教室モニターや電子黒板に投影する表示専用ページです。参加用QRコードは、授業前に自動表示され、授業中も操作バーから表示できます。カメラ映像の配置は[先生向けガイド「対面授業との併用」](docs/先生向けガイド.md#対面授業との併用)に記載しています |
...
| **AI要約**（任意） | 「反応」タブから、授業全体の文字起こしと反応が集中した箇所をもとに要約を作成します |
| **反応** | ボタン反応とコメントを時系列で表示し、それぞれの前後の音声を確認できます。アンケートとタスクの要旨も表示されます |
| **アンケート** | 設問ごとの集計を表示します。選択式は、締め切りまでに参加した生徒の未回答を含む円グラフ、記述式は名前付きの一覧です |
| **タスク** | タスクごとの完了人数と、1人目・参加者の半数・全員が完了した時刻を表示します。未到達の段階も確認できます。完了直後に取り消された操作は集計しません |
| **復習動画** | 指定した箇所を章として切り出し、選択した章を生徒へ公開できます。章は手動で追加でき、AIによる自動章分けも授業ごとに選択できます。公開画面には生徒の名前とコメントを表示しません |
...
| 画面 | 説明 |
|---|---|
| `/check` | 「端末チェック」では、端末1台の接続、回線速度、音声・映像形式を確認します。「料金の試算」では、授業回数、参加方法、AI機能から月額を計算します。どちらもログインは不要です |
...
再生できる形式は、端末とブラウザによって異なります。
このアプリはブラウザ名で決めず、接続時に端末が申告した対応形式を使用します。

```mermaid
flowchart TD
  A["生徒・モニターが接続"] --> B{"その端末が再生できる形式は?"}
  B -->|"WebMを再生できる"| C["WebM（Opus）"]
  B -->|"MP4を再生できる"| D["MP4（AAC）"]
  C --> E["サーバが、必要な形式だけを先生の端末に要求する"]
  D --> E
  E --> F["先生の端末は要求された形式だけを送る<br/>混在時だけ2本になる"]
```

ChromeのMediaRecorderで映像をMP4にした試験では、500msを指定した場合も、
実際にデータが生成される間隔は**約4.1秒**でした。同一条件で測定した遅延は、
**WebMが1.4秒、MP4が5.3秒**です。MediaRecorderのMP4だけに統一すると、
WebMを再生できる端末にも約4秒の遅延が加わります。

映像のMP4は、対応環境ではWebCodecsを使用して組み立て、0.5秒ごとに分割します（`client/src/lib/lowLatencyMp4.ts`）。
音声だけを配信する場合は、WebM（Opus）を優先します。WebMを再生できない端末には、
約0.5秒ごとに生成できるMP4（AAC）を送ります。
...
1台の開発機上で、現在より大きい48kbps相当の音声を使用して接続を模擬した負荷試験では、
音声のみで2000接続（0.26コア・96Mbps）、
映像を含む場合は300接続への300Mbpsの送信を確認しました。この試験は、同数の実端末を接続したものではありません。
模擬試験の範囲では、CPUとメモリの使用量は上限に達していません。同時接続数は、主にサーバの送信帯域によって制限されます。
...
| `/screen/:id?k=…` | 教室 | トークン付きの教室モニターURL（表示専用・ログイン不要） |
...
| `/check` | 誰でも | 端末チェックと料金の試算（`Ctrl+Alt+C` でどの画面からでも開く） |
...
![先生の授業一覧。ここから新しい授業を作り、授業画面を開く](docs/images/dashboard.png)
...
4. **機能の設定** — 使用するAI機能を選択します。授業を開始したあとは変更できません。
5. **授業の開始** — 「授業を開始」を押すと、配信と録音が始まります。スライド移動・ポインターなどは、この時刻以降の操作を記録します。開始前の書き込みは、開始0分時点の状態として記録します。
6. **授業中の操作** — 画面のボタン、キーボード、スライド一覧でページを移動し、必要に応じて書き込みます。コメント整理を使用する場合は、カードの種類と、質問に関係する説明が表示されます。
7. **授業の終了** — 「授業を終了」を押します。終了後は、`/review/:id`で授業を振り返ることができます。
...
**回線**：音声のみの配信には32kbpsを指定します。カメラ映像を受信する場合は、別途約900kbpsが必要です。
`/check`では、端末の受信速度と対応形式を測定し、スライド、音声、映像を使用できる目安を表示します。
先生の端末では、送信速度と、ブラウザが実際に生成するOpus・AAC音声の通信量も測定できます。
...
現在の構成では、サーバ本体と3GBの永続ディスクに月額の固定費が発生します。
このほかに、AI利用料と、Renderの無料枠を超えた外向き通信量が加わります。

[/check?tab=cost](https://enkaku-jugyou.onrender.com/check?tab=cost) の「料金の試算」では、
授業の長さ・回数・参加方法・PDF容量・使用するAI機能を入力して月額を確認できます。
為替は日本銀行の公表値を5円単位に丸めて使用します。取得できない場合は、画面にその旨と代替値を表示します。
内訳と注意事項は [導入を検討する方へ「月額の目安」](docs/導入を検討する方へ.md#月額の目安クラウド運用) にあります。
...
AI機能を使用する場合は、OpenAIのAPIキーを設定します。
初期状態では、コメント整理・字幕履歴の補正・授業全体の要約・復習動画の自動章分けが選択されています。
授業ごとに使用しない機能を外すこともできます。OpenAIのAPIキー1つで、文字起こしと要約を利用できます。
...
モデルの既定値は`OPENAI_MODEL`で変更できます（`server/src/config.ts`）。
開発時のモック応答と要約プロバイダーの切り替えは、
[ARCHITECTURE.md「AIプロバイダーの切り替え」](docs/ARCHITECTURE.md#aiプロバイダーの切り替え)に記載しています。
