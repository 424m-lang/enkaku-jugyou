...
カメラ映像では全員を同時に見ることができず、口頭で尋ねる方法では全員から同時には受け取れません。

このシステムは、**生徒がボタンを1つ押すだけで意思表示できる**手段を追加します。
...
| **その瞬間への聞き直し** | すべての反応を「授業開始からの経過ミリ秒」で記録しています |

生徒の画面はこれだけです。ボタンを押すか、コメントを送るかの2つしかありません。
下は、学校で配られる端末（1280×800）で開いたところです。
...
手続きが要るかどうかは、**こちらでは判断できません。**
ご確認いただく際の材料としてお使いください。
...
> 以前はこの2つが**録音を最初から起こし直していました**。5分の授業で計測したところ、
> 授業中に374秒ぶんを送ったうえに、授業後の2操作でさらに490秒ぶんを送っていました
> （録音時間の約2.7倍）。同じ操作での追加の呼び出しは、現在**0回**です。
...
**50分の授業1コマで 60円前後**です。うち50円ほどが文字起こしです。
...
先生が忘れた場合はサーバ側で設定し直します。授業の記録はアカウントに紐づいたまま残ります。
...
**未設定なら誰でもアカウントを登録できます**（ローカル運用や校内限定なら、未設定でも構いません）。
