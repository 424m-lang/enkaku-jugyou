...
授業後は、その瞬間に戻って振り返ったり、復習動画を作ったりできます。

先生も生徒もアプリのインストールは不要で、URLを開くだけで使えます。
**何ができるかは [機能](#機能)、導入の判断材料は
[導入を検討する方へ](docs/導入を検討する方へ.md) にあります。**
...
**音を出す端末を1台に絞る**必要があり、その理由と手順は
**[先生向けガイド「教室で対面と併用するとき」](docs/先生向けガイド.md#教室で対面と併用するとき)** にあります。

### 困ったときは

**症状別の対処は [先生向けガイド「困ったときは」](docs/先生向けガイド.md#困ったときは) にまとめています。**
音・画面・マイク・カメラ・字幕の5つに分けてあります。

どの症状でも、まず**端末チェック**（アドレスの最後を `/check` に変える）で切り分けられます。
接続・音声・マイクをその端末で実際に測り、○×で判定します。
...
<!-- ▼ 2026-08-28: コードを触るときの注意は ARCHITECTURE へ集約しました／編集どうぞ -->

### 実装上の落とし穴は ARCHITECTURE にあります

「なぜそう書いてあるのか」が分かりにくく、整理すると壊れる箇所がいくつかあります。
理由つきで [ARCHITECTURE.md「触るときに気をつけること」](docs/ARCHITECTURE.md#触るときに気をつけること) にまとめました。
スライドの描画枚数、無音とWhisper、用語ヒントの長さ、単一インスタンス前提などが含まれます。
...
コメントの前後だけを切り出す作りも考えましたが、**全文を必要とする機能が3つ**あるため、
結局あとで全体を起こすことになります。
...
**字幕のために追加の費用はかかりません。** ライブの字幕は先生の端末のブラウザが作り、
外部には送っていません。授業中に見る「字幕の履歴」も、授業後に見る「用語を直した字幕」も、
上の文字起こしを並べ直して使っているだけです。

**授業のあとの操作でも、追加の文字起こしはほとんど発生しません。**
授業中に貯めた分を組み合わせて使い、足りない範囲だけを補います（`server/src/ai/fullTranscript.ts`）。

費用の目安は [月額のめやす](#月額のめやす) にあります。
...
費用は「授業の長さ × コマ数」でほぼ決まり、要約やコメントの文字数はあまり効きません。
...
- **複数サーバインスタンスへのスケールアウトは未対応**（Socket.IOのアダプタ等が必要）。
  1台でどこまで届くかは
  [ARCHITECTURE.md「規模の頭打ちは、CPUではなく通信量です」](docs/ARCHITECTURE.md#規模の頭打ちはcpuではなく通信量です) に実測値があります
