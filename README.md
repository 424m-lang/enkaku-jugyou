...
配信するのは音声とスライドだけで、音声は 48kbps です。
...
カメラを使うのは**先生だけ**で、顔にも手元の実演にも使えます。**生徒が映ることはありません**。
...
4. **開始** — 「授業開始」で配信と録音が始まります。**ここからの時刻が、すべての記録の基準**です
5. **授業中** — スライドを送り、書き込みます。コメントが届くと、直前の説明を要約したカードが出ます
6. **終了** — 「授業終了」を押すと、`/review/:id` でいつでも振り返れます
...
**自動で戻さないのは、学校の共有端末を想定しているため**です。次に使う生徒が前の生徒になってしまいます。
...
同時に何本か走らせると、複数の生徒がボタンを押したときの先生画面を再現できます。
送るのはボタン反応だけなので、**コメントのカードは出ません**（生徒画面から送ると出ます）。
