...
生徒は名乗らずに、ボタンとコメントで反応できます。反応は**時刻とともに記録されます**。
そのため先生は、授業中にその場で反応を確認できるだけでなく、授業のあとに**その瞬間の説明へ戻って**確かめられます。
...
送った反応は、他の生徒には表示されません。先生の話を止めることもありません。
反応は時刻とともに記録します。授業のあとに、その瞬間へ戻って確認できます。

### このアプリでできること

| できること | 仕組み |
|---|---|
| **名乗らずにできる意思表示** | 生徒はボタンを1つ押します。名前の入力・発言・カメラは、いずれも必要ありません |
| **コメントへの気づき** | コメントが届くとカードが出ます。対応済みにするまで残りますので、手が空いたときにご覧いただけます |
| **その瞬間への聞き直し** | すべての反応を「授業開始からの経過時間」で記録しています |

生徒の画面はこれだけです。いつでもできるのは、ボタンを押すか、コメントを送るかの2つです。
これに、先生が出したときだけ**タスクのチェック**と**アンケートの回答**が加わります。
下図は、生徒用の画面を開いたところです。
...
カメラを使うのは**先生だけ**です。**生徒が映ることはありません**（生徒の画面には、カメラを使う機能がありません）。

先生のカメラは、**顔を見せる場合と、手元の実演を見せる場合の両方**を想定しています。
顔が見えるほうが授業として自然な場面も多いため、実演専用の機能とはしていません。
...
取得した情報を保護するのではなく、取得しなくても動くように機能を設計しています。
...
| **コメント分析** | コメントが届くと、AIが「先生がその直前に何を話していたか」を特定し、近い時刻の複数コメントを1枚のカードにまとめる。授業で扱っていない話題はそう表示する。カードは**対応済みにするまで残る**（[実際の出力](docs/先生向けガイド.md#5-授業中にできること)） |
...
| 高い音が鳴り続ける | ハウリング（音の回り込み）が考えられます。先生のマイクの近くで音を出している端末を「教室から参加」に切り替えてください。先生の画面は自分の声を鳴らさないため、音の発生源は別の端末にあります |
...
色と動物の組み合わせには、参加した順番が含まれません。それでいて「赤いキツネ」のように、一人ひとり異なる名前になります。
...
| 生徒のカメラ・マイク | **使いません**。生徒の映像と音声は、配信も録音もしません |
...
| 費用 | 0円 | 月 $7 程度〜（Render Starter。約1,100円）+ AI利用料。[内訳](#月額のめやすクラウド運用) |
| 校外から受講 | できない | できる |
...
生徒の端末は、**CPUを16分の1に絞っても**ページ送りに0.3秒で追いつきました。
性能が要るのは先生の端末だけです（カメラを使う場合）。
誰がどんな端末と回線を用意すればよいかは、
[導入を検討する方へ「まず、この表です」](docs/導入を検討する方へ.md#まずこの表です) にまとめています。
...
文字起こしは**音声の分数**で課金されます。そのためAIの費用は
**「授業の長さ × コマ数」でほぼ決まります**。

この50円には、**録音時間より1割強多く送る量**が含まれています。内訳は次のとおりです。
...
（カードを見たいときは、生徒の画面から実際にコメントを送ってください）。

### パスワードを設定し直す

メールアドレスを預かっていないため、画面からの「パスワードを忘れた」はありません。
...
- 音声配信に **WebSocket** を使います。対応するプラン・構成にしてください
...
- `SESSION_SECRET` を設定しないと、ソースに書かれた既定の文字列で先生のログインCookieに署名します。
  未設定のまま `NODE_ENV=production` で起動すると、起動時に警告を出します（`REGISTER_CODE` も同じです）
- `REGISTER_CODE` を設定しないと、**URLを知った人は誰でも先生アカウントを作れます**。
  授業データは先生ごとに分かれているため、他人の授業は見えません。ただし、AIの利用料はサーバの持ち主に発生します
...
**未設定の場合、URLをご存じの方はどなたでもアカウントを登録できる状態になります。ご注意ください。**
