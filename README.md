...
- 先生: `/register` でアカウント作成（ログインID＋表示名のみ。メールアドレス不要） → ダッシュボードでPDFをアップロードして授業作成
  - ※ パスワード再設定機能はありません。忘れた場合は新しいアカウントを作り直してください
...
## 先生向け: アイコンから起動できるようにする

このアプリはPWA（インストール可能なWebアプリ）に対応しています。WordやExcelのように、アイコンから直接起動できます。

- **パソコン（Edge / Chrome）**: アプリのURLを開く → アドレスバー右端の「インストール」アイコン（画面のような形）をクリック → 以後はスタートメニュー・タスクバー・デスクトップから起動可能（独立ウィンドウで開きます）
  - ボタンが見当たらない場合: ブラウザ右上の「…」メニュー →「アプリ」→「このサイトをアプリとしてインストール」
- **スマホ（生徒向け）**: URLを開く → 共有メニュー →「ホーム画面に追加」
- 単なるショートカットでよければ、デスクトップ右クリック →「新規作成」→「ショートカット」にURLを貼るだけでも起動アイコンになります

アイコン画像を変更したい場合は `client/public/icon.svg` を編集し、`node scripts/generate-icons.mjs` で再生成してください。
...
- **プライバシー（データ最小化）**: システム全体で個人情報を必須としない。先生は任意のログインID＋表示名（ニックネーム可）で登録でき、メールアドレス等は収集しない。生徒はアカウント不要・表示名のみで参加。Phase 2（同意管理・匿名化）用に `participants.consent_status` と `lessons.anonymize_mode` をスキーマに用意済み（ロジックは未実装）
