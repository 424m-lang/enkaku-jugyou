...
export async function suggestReflectionFocus(
  ...
  return {
    text: `（モック提案）直近で${info.participantCount}人の反応が集中したのは、この文字起こし区間の内容です。ここを重点的に振り返ることをお勧めします。SUMMARY_PROVIDER=anthropic と ANTHROPIC_API_KEY を設定すると実際のAI提案になります。`,
    provider: 'mock',
  };
}
...
export async function summarizeLesson(
  ...
  return {
    text: [
      '## 授業の概要（モック要約）',
      'ANTHROPIC_API_KEY を設定すると、文字起こし全体からAIによる実際の要約が生成されます。',
      '',
      '## 生徒の反応が集中した箇所',
