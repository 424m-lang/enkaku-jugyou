 ...
 * 全員がWebMを再生できるならWebMだけ、MP4だけに対応する端末ならMP4だけ、
 * **両方の形式が必要な受け手が混ざっていれば両方を同時に流す**。
 *
 ...
 * MediaRecorderのMP4へ全体を揃えると、WebMを再生できる端末にも約4秒の遅延が加わる。
 ...
 *
 * 音声のみの配信にはこの問題が無い。音声はWebM/Opusを優先し、
 * WebMを再生できない端末へ送るAACも約0.5秒ごとに生成できる。
 *
...
export async function startCameraBroadcast(
   ...
   * 例えばWebMを録れない環境では、WebMを求められてもMP4に切り替わる。
